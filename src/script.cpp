...
CBigNum CastToBigNum(const valtype& vch)
{
    ...
    // Get rid of eDTEPa leading zeros
...
bool EvalScript(vector<vector<unsigned char> >& stack, const CScript& script, const CTransaction& txTo, unsigned int nIn, int flags, int nHashType)
{
    CAutoBN_CTX pctx;
                ...
                case OP_NOP2:
    {
        ...
        break;
                ...
                //case OP_NOTEQUAL: // use OP_NUMNOTEQUAL
                {
                    ...
                    // something like n != 1 and have some wiseguy pass in 1 with eDTEPa
...
uint256 SignatureHash(CScript scriptCode, const CTransaction& txTo, unsigned int nIn, int nHashType)
{
    ...
    // or an eDTEPa one at the end, this prevents all those possible incompatibilities.
...
bool EDTEPactDestination(const CScript& scriptPubKey, CTxDestination& addressRet)
{
    vector<valtype> vSolutions;
    txnouttype whichType;
    if (!Solver(scriptPubKey, whichType, vSolutions))
...
public:
    ...
    void Process(const CScript &script) {
        ...
        int nRequired;
        if (EDTEPactDestinations(script, type, vDest, nRequired)) {
...
void EDTEPactAffectedKeys(const CKeyStore &keystore, const CScript& scriptPubKey, std::vector<CKeyID> &vKeys) {
    CAffectedKeysVisitor(keystore, vKeys).Process(scriptPubKey);
}

bool EDTEPactDestinations(const CScript& scriptPubKey, txnouttype& typeRet, vector<CTxDestination>& addressRet, int& nRequiredRet)
{
    addressRet.clear();
    typeRet = TX_NONSTANDARD;
    vector<valtype> vSolutions;
    ...
    else
    {
        ...
        if (!EDTEPactDestination(scriptPubKey, address))
           return false;
        addressRet.push_back(address);
    }
...
{
    // EDTEPa-fast test for pay-to-script-hash CScripts:
    return (this->size() == 23 &&
...
bool EDTEPactPubKey(const CScript& scriptPubKey, const CKeyStore* keystore, vector<unsigned char>& vchPubKeyRet)
{
    vchPubKeyRet.clear();
...
bool EDTEPactHash160(const CScript& scriptPubKey, uint160& hash160Ret)
{
    hash160Ret = 0;
...
{
    uint160 hash160;
    if (EDTEPactHash160(*this, hash160))
        return hash160;
    vector<unsigned char> vch;
    if (EDTEPactPubKey(*this, NULL, vch))
        return Hash160(vch);
    return 0;
}
