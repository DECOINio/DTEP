...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "DECOIN is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "DECOIN is downloading blocks...");
    ...
    if (params.size() == 0)
    {
        ...
        // Update nEDTEPaNonce
        static unsigned int nEDTEPaNonce = 0;
        IncrementEDTEPaNonce(pblock, pindexPrev, nEDTEPaNonce);
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "DECOIN is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DECOIN is downloading blocks...");
