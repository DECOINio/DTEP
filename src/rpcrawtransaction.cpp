...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, Object& out, bool fIncludeHex)
{
    ...
    if (!EDTEPactDestinations(scriptPubKey, type, addresses, nRequired))
    {
...
Array listunspent__(double p, double& iR)
{
    ...
    BOOST_FOREACH(const COutput& out, vecOutputs)
    {
        ...
        if(setAddress.size())
        {
            ...
            if(!EDTEPactDestination(out.tx->vout[out.i].scriptPubKey, address))
                continue;
...
Value listunspent(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 2)
    {
        ...
        BOOST_FOREACH(Value& input, inputs)
        {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid DECOIN address: ")+input.get_str());
            if (setAddress.count(address))
