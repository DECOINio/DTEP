...
Value primaryCXValidate(const Array& params, bool fHelp)
{
    ...
    if(!externPredicate.IsValid())
    {
      ...
      {
          throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid DECOIN predicate");
      }
    }
    ...
    CBitcoinAddress localPredicate(l0Str);
...
Value transferEncryptedExtPredicate(const Array& params, bool fHelp)
{
    ...
    CBitcoinAddress externPredicate((params[3]).get_str());
    if(!externPredicate.IsValid())
