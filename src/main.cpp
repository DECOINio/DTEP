...
static const int nStakeStartHeight = 24 * 60 * 60; // after 2 months
...
unsigned int nStakeMaxAge = 24 * 60 * 60 * 365; // 1 year
...
const string strMessageMagic = "DECOIN Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx)
{
    ...
    // EDTEPemely large transactions with lots of inputs can cost the network
...
bool CTransaction::AreInputsStandard(const MapPrevTx& mapInputs) const
{
    ...
    if (aliasScript(prevScript, op, vvch, pc))
        ...
        // Transactions with eDTEPa stuff in their scriptSigs are
...
{
	if(nHeight <= nStakeStartHeight)
		return nFees;
    int64_t nSubsidy = 6.2 * nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);
	if (fDebug && GetBoolArg("-printcreation"))
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    if (!CheckStakeModifierCheckpoints(pindexNew->nHeight, pindexNew->nStakeModifierChecksum))
	{
       ...
       return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016, test=%08x"PRIx64, pindexNew->nHeight, nStakeModifier, pindexNew->nStakeModifierChecksum);
	}
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    {
        // EDTEPa checks to prevent "fill up memory by spamming with bogus blocks"
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DECOIN", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "DECOIN is Launched";
        CTransaction txNew;
        txNew.nTime = 1523615744;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1523615744;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
		block.nNonce   = !fTestNet ? 824269 : 1;
/*
        if ((block.hashMerkleRoot != hashGenesisMerkleRoot) || (block.GetHash() != (!fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet)))
        {
...
unsigned char pchMessageStart[4] = { 0x57, 0x9F, 0xB5, 0xEF };
